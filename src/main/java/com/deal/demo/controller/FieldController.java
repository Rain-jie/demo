...
public class FieldController extends ControllerException {
     ...
     * @returnA
     */
    @RequestMapping(value = "/phone/getField", method = RequestMethod.POST)
    public ResponseHead getField(HttpServletResponse response,
        ...
        Map<String,Object> user = fieldService.getField(userMap);
     ...
     * @return
     */
    @RequestMapping(value = "/phone/registerField", method = RequestMethod.POST)
    public ResponseHead registerField(HttpServletResponse response,
         ...
         * 业务处理
         */
        Map<String,Object> user = fieldService.registerField(userJsonMap);
        if (user.get("type").toString().contains("error&")) {
            return ResponseHttpStatus.errorRequest(responseHead, user.get("type"), "interfaceStatus");
        }
        return ResponseHttpStatus.successRequest(responseHead, user, "interfaceStatus");
     ...
     * @return
     */
    @RequestMapping(value = "/phone/updateField", method = RequestMethod.POST)
    public ResponseHead updateField(HttpServletResponse response,
                                       @RequestBody Map<String, Object> userJsonMap) {
        //获取请求状态码
        int responseStatus = response.getStatus();
        //如果状态码等于200代表成功,否则代表失败
         ...
         * 业务处理
         */
        Map<String,Object> user = fieldService.updateField(userJsonMap);
        if (user.get("type").toString().contains("error&")) {
            return ResponseHttpStatus.errorRequest(responseHead, user.get("type"), "interfaceStatus");
        }
        return ResponseHttpStatus.successRequest(responseHead, user, "interfaceStatus");
     ...
     * @return
     */
    @RequestMapping(value = "/phone/deleteField", method = RequestMethod.GET)
    public ResponseHead deleteField(HttpServletResponse response,
         ...
         * 业务处理
         */
        Map<String,Object> user = fieldService.deleteField(fieldId);
        if (user.get("type").toString().contains("error&")) {
            return ResponseHttpStatus.errorRequest(responseHead, user.get("type"), "interfaceStatus");
        }
        return ResponseHttpStatus.successRequest(responseHead, user, "interfaceStatus");
